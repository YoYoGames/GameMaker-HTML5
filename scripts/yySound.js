...
class AudioBus
{
	...
	constructor()
	{
		...
		this.bypass = false;
		this.gain = 1.0;
		this.effects = Array(AudioBus.NUM_EFFECTS).fill(undefined);

		// This fixes the array's size (but elements are still mutable)
		Object.seal(this.effects);

		Object.defineProperties(this, {
			gmlbypass: {
				enumerable: true,
				get: function () { return this.bypass; },
				set: function (_state) { this.bypass = yyGetBool(_state); }
			},        
			gmlgain: {
				enumerable: true,
				get: function () { return this.gain; },
				set: function (_gain) { this.gain = yymax(0.0, yyGetReal(_gain)); }
			},
			gmleffects: {
				enumerable: true,
				...
				set: function (_effects) { 
					if (_effects instanceof Array
						&& _effects.length == AudioBus.NUM_EFFECTS)
					{
						this.effects = _effects;
					}
					else
					{
						throw new Error("AudioBus.effects must be an array of size " + AudioBus.NUM_EFFECTS);
					}
				}
			}
		});
